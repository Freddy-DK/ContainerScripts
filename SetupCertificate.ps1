...
try {
    ...
    Write-Host "Get Service Directory"
    Get-ACMEServiceDirectory -State $stateDir -ServiceName "LetsEncrypt" -PassThru
    $stateDir | Out-Host

    Write-Host "Creating New Nonce"
    New-ACMENonce -State $stateDir

    Write-Host "Creating New AccountKey"
    New-ACMEAccountKey -state $stateDir -PassThru

    Write-Host "Creating New Account"
    New-ACMEAccount -state $stateDir -EmailAddresses $ContactEMailForLetsEncrypt -AcceptTOS
...
' | Set-Content (Join-Path $challengeLocalPath 'web.config')
    ...
    Import-PfxCertificate -Password $SecurePfxPassword -FilePath $certificatePfxFile -CertStoreLocation "cert:\localMachine\TrustedPeople" -Exportable | Out-Null
...
Get-Website | Where-Object { $_.Name -eq 'challenge' } | ForEach-Object {
...
# Configure WinRM on port 5986 using certificate
winrm delete winrm/config/listener?Address=*+Transport=HTTP
$httpsConfig = "@{Hostname=""$($publicDnsName)"";CertificateThumbprint=""$($cert.Thumbprint)""}"
winrm create winrm/config/listener?Address=*+Transport=HTTPS $httpsConfig
winrm set winrm/config/service/auth '@{Basic="true"}'
winrm get winrm/config/service
winrm enumerate winrm/config/listener
Restart-Service WinRM

# Add BC user as container admin
